<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8">
    
    <title>A Twosome Place</title>
    <link rel="icon" href="img/footerLogo.png">
    <link rel="stylesheet" href="css/reset.css">
    <link rel="stylesheet" href="css/common.css">
    <link rel="stylesheet" href="css/sub3.css">
</head>

<body>
    <div class="menuWrap">
    	<div class="menu">
    		<ul>
    			<li>로그인</li>
    			<li>회원가입</li>
    			<li>고객센터</li>
    			<li>공지사항</li>
    		</ul>
    	</div>
    </div>
    <div class="hdrWrap">
    	<div class="header">
    		<h1><a href="index.html"></a></h1>
    		<ul>
    			<li><a href="sub1.html">브랜드 스토리</a></li>
    			<li><a href="sub2.html">메뉴</a></li>
    			<li><a href="sub3.html">이벤트</a></li>
    			<li><a href="">가맹정보</a></li>
    			<li><a href="">매장정보</a></li>
    		</ul>
    	</div>
    </div>
    <div class="subTitle">
        <div class="segment">
            <h2>이벤트&amp;프로모션</h2>
        </div>
    </div>
    <div class="main">
        <div class="mainImg"></div>
    </div>

    <div class="nav">
        <div class="btn">
            <button><a href="#progressEvent">진행중인 이벤트</a></button>
        </div>
        <div class="btn">
            <button>매장별 이벤트</button>
        </div>
        <div class="btn">
           
            <button><a href="#endEvent">종료된 이벤트</a></button>
            
        </div>
    </div>
    <div class="content cnt1" id="progressEvent">
        <div class="header">
            <h3>진행중인 이벤트 &amp; 프로모션</h3>
        </div>
        <div class="section">
            <div class="article">
                <div class="image i1"></div>
                <div class="segment">
                    <h5>아메리카노 50%할인</h5>
                    <p>2021-01-27 ~ 2021-02-09</p>
                </div>
            </div>
            <div class="article">
                <div class="image i2"></div>
                <div class="segment">
                    <h5>투썸 X 무직타이거 콜라보</h5>
                    <p>2021-01-27 ~ 2021-02-09</p>
                </div>
            </div>
            <div class="article">
                <div class="image i3"></div>
                <div class="segment">
                    <h5>속마음 연구소</h5>
                    <p>2021-01-27 ~ 2021-02-09</p>
                </div>
            </div>
            <div class="article">
                <div class="image i4"></div>
                <div class="segment">
                    <h5>투썸 모닝과 함께 하는 아침</h5>
                    <p>2021-01-27 ~ 2021-02-09</p>
                </div>
            </div>
            <div class="article">
                <div class="image i5"></div>
                <div class="segment">
                    <h5>투썸 화이트데이</h5>
                    <p>2021-01-27 ~ 2021-02-09</p>
                </div>
            </div>
            <div class="article">
                <div class="image i6"></div>
                <div class="segment">
                    <h5>투썸 홀케이크 할인쿠폰</h5>
                    <p>2021-01-27 ~ 2021-02-09</p>
                </div>
            </div>
            <div class="article">
                <div class="image i7"></div>
                <div class="segment">
                    <h5>Spacial Treat For you</h5>
                    <p>2021-01-27 ~ 2021-02-09</p>
                </div>
            </div>
            <div class="article">
                <div class="image i8"></div>
                <div class="segment">
                    <h5>인기메뉴 6종 응모 이벤트</h5>
                    <p>2021-01-27 ~ 2021-02-09</p>
                </div>
            </div>
        </div>
    </div>
    <div class="content cnt2" id="endEvent">
        <div class="header">
            <h3>종료된 이벤트</h3>
        </div>
        <div class="section">
            <div class="article">
                <div class="image i1"></div>
                <div class="segment">
                    <h5>숨겨진 미스터리 기프트를 찾아라!</h5>
                    <p>2021-01-27 ~ 2021-02-09</p>
                </div>
            </div>
            <div class="article">
                <div class="image i2"></div>
                <div class="segment">
                    <h5>카카오톡 선물하기 X 투썸플레이스</h5>
                    <p>2021-01-27 ~ 2021-02-09</p>
                </div>
            </div>
            <div class="article">
                <div class="image i3"></div>
                <div class="segment">
                    <h5>투썸 치즈 디저트 &amp; 아메리카노</h5>
                    <p>2021-01-27 ~ 2021-02-09</p>
                </div>
            </div>
            <div class="article">
                <div class="image i4"></div>
                <div class="segment">
                    <h5>감사의 달 이벤트</h5>
                    <p>2021-01-27 ~ 2021-02-09</p>
                </div>
            </div>
            <div class="article">
                <div class="image i5"></div>
                <div class="segment">
                    <h5>투썸 베스트 디저트</h5>
                    <p>2021-01-27 ~ 2021-02-09</p>
                </div>
            </div>
            <div class="article">
                <div class="image i6"></div>
                <div class="segment">
                    <h5>크리스마스 케이크 할인</h5>
                    <p>2021-01-27 ~ 2021-02-09</p>
                </div>
            </div>
            <div class="article">
                <div class="image i7"></div>
                <div class="segment">
                    <h5>투썸 푸딩출시 이벤트</h5>
                    <p>2021-01-27 ~ 2021-02-09</p>
                </div>
            </div>
            <div class="article">
                <div class="image i8"></div>
                <div class="segment">
                    <h5>가을맞이 아메리카노 1+1</h5>
                    <p>2021-01-27 ~ 2021-02-09</p>
                </div>
            </div>
        </div>
        <menu>
            <i class="fas fa-chevron-circle-down"></i>
        </menu>
    </div>
    <div class="ftrWrap">
    <div class="footer">
        <div class="header">
            <ul>
                <li>오시는 길</li>
                <li>제휴문의</li>
                <li>개인정보처리방침</li>
                <li>이용약관</li>
                <li>고객센터</li>
            </ul>
        </div>
    </div>
    <hr>
    <div class="section">
        <h1><a href=""></a></h1>
        <p>
            서울특별시 중구 을지로 170,8층(을지로4가, 을지로트윈타워) 투썸플레이스(주)&nbsp;&nbsp;&nbsp;대표이사:이영상
            <br>사업자등록번호 : 404-86-01054&nbsp;&nbsp;&nbsp;helpmaster@twsome.co.kr
        </p>
        <p>
            Copyright ⓒ 2020 A TWOSOME PLACE CO.LTD.ALL RIGHTS RESERVED.
        </p>
        <menu>
            <i class="fab fa-facebook-square"></i>
            <i class="fab fa-instagram-square"></i>
            <i class="fab fa-youtube-square"></i>
        </menu>
    </div>
</div>


</body></html>
